...
def main():
    ...
    for cell_id in range(num_cells):
        ...
        print(f"Running optimization for cell {cell_id}")
        optimized_params = stan_session.model.optimizing(data=calcium_data)
    ...
    # plot original trajectories vs. simulated
    y_combined = np.dstack((y[:num_cells, t0 + 1:], y_sim))
    pdf_multi_plot(plt.plot, y_combined,
                   os.path.join(result_dir, "trajectories.pdf"))
