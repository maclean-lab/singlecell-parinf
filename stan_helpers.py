        ...
        with PdfPages(output_path) as pdf:
            for page in range(num_pages):
                ...
                for plot_idx in range(num_subplots):
                    ...
                    param_samples = []
                    for analyzer in self.session_analyzers:
                        session_samples = [s.values[:, param_idx]
                                            for s in analyzer.samples]
                        param_samples.append(
                            np.concatenate(session_samples))
    ...
    def run_sensitivity_test(self, ode, t0, ts, y0, y_ref, target_var_idx,
        ...
        def _test_quantile(q):
            ...
            for i, pct in enumerate(test_percentiles):
                ...
                # simulate if we want to compare to sampled trajectory
                if ref_traj_type == 'sampled':
                    y_pct = simulate_trajectory(ode, theta, t0, ts, y0[idx, :])
                    y_pct = y_pct[:, target_var_idx]
                else:
                    y_pct = y_ref[idx, :]

                peak_pct = np.max(y_pct)
                steady_pct = np.mean(y_pct[-num_steady_pts:])

                # perturb current draw
                ...
                traj_dist += np.linalg.norm(y_sim - y_pct)

                # get stats for change in peak
                peak_sim = np.max(y_sim)
                peak_diff += peak_sim - peak_pct
                if peak_pct > peak_sim:  # negative fold change
                    peak_fold_change -= peak_pct / peak_sim
                else:  # positive fold change
                    peak_fold_change += peak_sim / peak_pct

                # get stats for change in steady state
                steady_sim = np.mean(y_sim[-num_steady_pts:])
                steady_diff += steady_sim - steady_pct
                if steady_pct > steady_sim:  # negative fold change
                    steady_fold_change -= steady_pct / steady_sim
                else:  # positive fold change
                    steady_fold_change += steady_sim / steady_pct
        ...
        for idx, analyzer in enumerate(self.session_analyzers):
            ...
            session_samples = analyzer.get_samples()
            if test_value == 'mode':
                sample_modes = analyzer.get_sample_modes(**sample_mode_kwargs)
